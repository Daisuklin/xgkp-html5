...
//	})
	...
	$('.park_bg').on('click', function() {
		$('.park_bg').toggle();
		$('.choice_guibi').toggle();
	})
    $('.park_bg2').on('click', function() {
        $('.park_bg2').toggle();
        $('.choice_noprice').toggle();
    })
	//支付金额全部被金币抵消为0时弹窗
	//	$('#choice_gcoin').on('click', function() {
	//		$('.park_bg').toggle();
	...
	//	})
	//关闭支付金额全部被金币抵消为0时弹窗
	$('.choice_noprice .btn_parkclose').on('click', function() {
		$('.park_bg2').toggle();
		$('.choice_noprice').toggle();
	})
...
//选择金币弹窗
function choiceGcoin(){
	$('.park_bg').toggle();
	$('.choice_guibi').toggle();
	document.getElementById('content_box').style.display ='none';//隐藏键盘
	document.getElementById('park_bg2').style.display ='none';//隐藏支付金额全部被金币抵消为0时弹窗
	document.getElementById('choice_noprice').style.display ='none';//隐藏支付金额全部被金币抵消为0时弹窗
}
//弹出支付金额全部被金币抵消为0时弹窗
function shownoprice() {
	...
	$('.choice_noprice').toggle();
	document.getElementById('content_box').style.display ='none';//隐藏键盘
	document.getElementById('park_bg').style.display ='none';//隐藏选择金币悬浮窗
	document.getElementById('choice_guibi').style.display ='none';//隐藏选择金币悬浮窗
}
...
//		if(spriceValue < 0.01) {
//			showError('金额不能小于0.01，请重新输入');
//			th.value = '';
//		} else if(spriceValue > 50000) {
